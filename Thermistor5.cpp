// Thermistor5.cpp member functions of class. For Arduino 0018 or newer.
...
void Thermistor5::ReadADC(unsigned int ExternalReading) {
...
void Thermistor5::CalculateTemperature() {
double LnResist;
...
//Calculate the ADC reading into temperature in Kelvin. (official Steinhart-Hart equations or B Coefficient Method)
//Steinhart is split up (Selection) to allow for comparisons and easy selection of the
...
void Thermistor5::CalculateTemperature(unsigned char Selection) {
if(this->BValue != 0){
  this->Temperature = (1.0/((float)this->NominalTemp + 273.15)) + (1/(float)this->BValue)*log(this->ResistanceThermistor/(float)this->NominalResistance);
  //Serial.print("Test: ");Serial.println(this->Temperature, DEC);
  //Final part is to invert
  this->Temperature = (1.0 / this->Temperature) + this->Offset;
}else{
  float LnResist;
  LnResist = log(this->ResistanceThermistor/(float)this->NominalResistance); // no reason to calculate this multiple times.
  //Serial.print("lnresist: ");Serial.println(LnResist, DEC);
...
//external ADC.
void Thermistor5::ReadCalculate(unsigned char Selection) {
this->ReadADC(0); // 0 means use internal ADC.
this->CalculateTemperature(Selection);
} //end ReadCalculate()


//Read analog and apply mode filter, then calculate temperature.  Mode filter function thanks to
//elcojacobs.com (http://www.elcojacobs.com/eleminating-noise-from-sensor-readings-on-arduino-with-digital-filtering/)
void Thermistor5::ReadCalculateModeFilter(unsigned char Selection, unsigned int num_reads){
  // read multiple values and sort them to take the mode
   ...
   this->ReadADC(ADCAverage);
   this->CalculateTemperature(Selection);
} //end ReadCalculateModeFilter()
...
double Thermistor5::GetCentigrade() {
...
//Since many will want one system or the other, this is split apart.
double Thermistor5::GetFarenheit() {
...
void Thermistor5::Thermistor5SerialPrint() {
  ...
  Serial.print("sizeof(Thermistor5) object: "); Serial.println(sizeof(Thermistor5),DEC);
  ...
  Serial.print(", ");
  Serial.println((this->GetFarenheit()-mytempf), DEC);
} //end Thermistor5SerialPrint()
#endif

//end of Thermistor5() class.
