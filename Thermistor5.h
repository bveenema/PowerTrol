// Thermistor5.h definition of class. For Arduino 0018 or newer.
// Thermistor class to consolidate various variables and functions.
// (c)20101017 by MODAT7 under GNU GPLv1
// (c)20140406 by VEEN under GNU GPLv3

/* 
///////Thermistor4 ---> Thermsitor5/////////
This is an update to the amazing Thermistor 4 Library.  This update allows you to use the
datasheet values for a thermistor by specifying the nominal restistance.  You will need to
set this parameter when initializing your resistor (ie. Thermistor4.Nominal where Thermistor4
is whatever you have named your object)

It also adds a mode filter for analog reads.  This is useful for long thermistor leads and noisy
environments. This is an additional function, so if not desired, you can ignore it or comment it 
out to save space.
Currently only works with internal analog readings:

ReadCalculateModeFilter(unsigned char Selection, unsigned int num_reads)
  where "Selection" is the 1-3 value used in CalculateTemperature()
  and "num_reads" is the total number of analog reading to take.
  the filter will sort the readings and average the middle 10.
...
Arduino install directory, "libraries" subdirectory, and create a "Thermistor5" directory. Copy
in Thermistor5.h and Thermistor5.cpp. Restart the Arduino IDE if it is running.

To use this library in a single project, exit the Arduino IDE if it is running and copy in
Thermistor5.h and Thermistor5.cpp into the project directory. It will show up in the extra tabs
when the IDE is relaunched. Add a #include "Thermistor5.h" into the sketch.
...
Thermistor5 ThermistorInside, ThermistorOutside;
...
#ifndef Thermistor5_H
#define Thermistor5_H
...
unsigned long NominalResistance; //Nominal Resistance of the thermistor
unsigned int NominalTemp;	//Temperature at which the nominal resistance is measured
unsigned int BValue;	//B Value of thermitor
...
//Thermistor5(); //Constructor, removed for size.
void SetUp();
void ReadADC(unsigned int);
void CalculateTemperature(unsigned char);
void ReadCalculate(unsigned char);
void ReadCalculateModeFilter(unsigned char, unsigned int);
double GetCentigrade();
double GetFarenheit();
#if THERMISTORDEBUG
void Thermistor5SerialPrint();
#endif
};

#endif

//end of Thermistor5.h
